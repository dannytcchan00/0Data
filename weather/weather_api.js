            ...
            for (let i = 0; i < stations.length; i++) {
                ...
                if (!coords) {
                    for (let key in macauCoordsMap) {
                        if (stationName.includes(key) || key.includes(stationName)) { coords = macauCoordsMap[key]; break; }
                    }
                }
                if (!coords) continue;
                ...
                if (type === 'temp') {
                    let tempStr = getValueFromTag("Temperature"); if (tempStr) val = parseFloat(tempStr);
                } else if (type === 'wind') {
                    let speedStr = getValueFromTag("WindSpeed"); if (speedStr) val = parseFloat(speedStr);
    ...
    try {
        ...
        if(rainList.length === 0) listHtml = `<div style="text-align:center; padding:20px; color:var(--text-muted);">暫時沒有雨量數據</div>`;
            ...
            rainList.forEach(item => {
                ...
                const colors = ['#3498db', '#2ecc71', '#f39c12', '#e67e22', '#e74c3c']; let barHtml = '';
        ...
        if (warnRes.status === 'fulfilled' && warnRes.value.ok) {
            ...
            if (warnData && typeof warnData === 'object') {
                for (let key in warnData) {
                    ...
                    else if (text.includes("火災") || text.includes("fire")) { if (text.includes("黃")) finalCode = "WFIREY"; else finalCode = "WFIRER"; }
