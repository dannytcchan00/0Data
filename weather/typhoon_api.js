...
function drawTyphoonTrack(points, mapLayerGroup, colorCode, agencyName, drawCone, isPrimary) {
    ...
    // 漏斗與覆蓋圈設定 (顏色跟隨該國機構，或者預設 JMA 淡黃色)
    const coneColor = colorCode; 
    const coneFill = hexToRgba(colorCode, 0.15); 
    ...
    if (drawCone) {
        ...
        if (points.length > 1) {
            ...
            L.polygon(funnelPolygonCoords, { color: coneColor, weight: 1.5, dashArray: '5, 5', fillColor: coneFill, fillOpacity: 0.25 }).addTo(mapLayerGroup);
        }
    }
    ...
    points.forEach((pt, idx) => {
        ...
        if (idx === 0) {
            if (isPrimary) {
                let spinningPin = L.divIcon({ className: '', html: `<div class="spinning-typhoon-icon" style="color:${colorCode};">🌀</div>${labelHtml}`, iconSize: [40, 40], iconAnchor: [20, 20] });
...
function renderAgencyMap() {
    ...
    if (currentSelectedAgency === 'ALL') {
        ...
        if (!globalParsedAgencyTracks['JMA'] || globalParsedAgencyTracks['JMA'].length === 0) {
            baselineAgency = Object.keys(globalParsedAgencyTracks)[0]; // Fallback if no JMA
        }
