...
// 改為只標示「香港」
let hkCenterPin = L.divIcon({ 
    className: '', 
    html: `<div style="background:#121212; color:#fff; font-size:0.75rem; font-weight:800; padding:2px 6px; border-radius:4px; border:1px solid #fff; box-shadow: 2px 2px 0px #000; letter-spacing:1px;">香港</div>`, 
    iconAnchor: [16, 12] 
});
...
L.marker(hkoCenter, {icon: hkCenterPin, zIndexOffset: 9999}).addTo(tcMapHko);
...
L.marker(hkoCenter, {icon: hkCenterPin, zIndexOffset: 9999}).addTo(tcMapAgency);

// 繪製香港距離圈函數 (1200km, 800km, 400km)
function drawDistanceRings(mapObj) {
    L.circle(hkoCenter, { radius: 1200000, color: '#3498db', weight: 1.5, fillColor: '#3498db', fillOpacity: 0.05, dashArray: '6, 6', interactive: false }).addTo(mapObj);
    L.circle(hkoCenter, { radius: 800000, color: '#f39c12', weight: 1.5, fillColor: '#f39c12', fillOpacity: 0.05, dashArray: '6, 6', interactive: false }).addTo(mapObj);
    L.circle(hkoCenter, { radius: 400000, color: '#e74c3c', weight: 2, fillColor: '#e74c3c', fillOpacity: 0.05, dashArray: '4, 4', interactive: false }).addTo(mapObj);
}

// 於兩個颱風地圖加上距離圈
drawDistanceRings(tcMapHko);
drawDistanceRings(tcMapAgency);
